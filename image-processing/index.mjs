...
const inDir = path.join(__dirname, './images/in/norcal');
const outDir = path.join(__dirname, './images/out/norcal');
...
async function processFile(file) {
	...
	try {
		...
		console.log({
			hasLocation,
			GPSLatitude: imageTags?.GPSLatitude,
			GPSLongitude: imageTags?.GPSLongitude,
		});
		if (!hasLocation) {
		...
		if (formatted !== undefined) {
			...
			try {
				...
				// console.log(`\n++++ FIND NEARBY GEO ++++`)
				if (!nearby?.geonames?.length) {
					console.log(chalk.redBright.bold(JSON.stringify(nearby, null, 2)));
					process.exit(1);
				}
...
const main = async () => {
	try {
		...
		const files = (await fs.readdir(inDir)).filter(f => {
			return f.indexOf('.jpeg') > -1 || f.indexOf('.mov') > -1;
		});
