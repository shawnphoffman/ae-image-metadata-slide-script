...
if (app.settings.haveSetting(scriptSection, scriptSettingStrings.compsFolder)) {
	var s = app.settings.getSetting(scriptSection, scriptSettingStrings.compsFolder);
	try {
...
if (app.settings.haveSetting(scriptSection, scriptSettingStrings.finalComp)) {
	var s = app.settings.getSetting(scriptSection, scriptSettingStrings.finalComp);
...
var dialog = (function () {
	...
	panel2.text = compsFolder ? 'Comps Folder: ' + compsFolder.name : 'Not set...';
	...
	button1.onClick = function () {
		...
		if (selection instanceof FolderItem) {
			...
			app.settings.saveSetting(scriptSection, scriptSettingStrings.compsFolder, selection.id);
		}
	};
	...
	var createCompDuration = group1.add('edittext {properties: {name: "edittext1"}}');
	...
	var createLayerScale = group1.add('edittext {properties: {name: "edittext2"}}');
	...
	var createPrefix = group2.add('edittext {properties: {name: "createPrefix"}}');
	...
	var selectedFontSize = group4.add('edittext {properties: {name: "selectedFontSize"}}');
	...
	var textLocationTop = group4.add('edittext {properties: {name: "textLocationTop"}}');
	...
	var textLocationLeft = group4.add('edittext {properties: {name: "textLocationLeft"}}');
	...
	button3.size = [20, 20];
	button3.fillBrush = button3.graphics.newBrush(button3.graphics.BrushType.SOLID_COLOR, [1, 1, 1]);
	...
	var selectedFontFamilyDate = panel7.add('dropdownlist', undefined, undefined, {
		name: 'selectedFontFamilyDate',
		items: fontFamilies,
	});
	selectedFontFamilyDate.helpTip = 'Font Family';
	selectedFontFamilyDate.selection = selectedFontFamilyDate.find(defaults.fontNameDate);
	...
	var selectedFontSizeDate = group6.add('edittext {properties: {name: "selectedFontSizeDate"}}');
	...
	var textDateLeft = group6.add('edittext {properties: {name: "textDateLeft"}}');
	...
	button5.size = [20, 20];
	button5.fillBrush = button5.graphics.newBrush(button5.graphics.BrushType.SOLID_COLOR, [1, 1, 1]);
	...
	buttonSetOutputComp.onClick = function () {
		...
		if (selection instanceof CompItem) {
			...
			app.settings.saveSetting(scriptSection, scriptSettingStrings.finalComp, selection.id);
		}
	};
	...
	var secondsPerImage = group9.add('edittext {properties: {name: "secondsPerImage"}}');
	...
	var secondsPerTransition = group9.add('edittext {properties: {name: "secondsPerTransition"}}');
	...
	function createCompFromItem(item) {
		...
		if (item instanceof FootageItem && /\.(jpg|jpeg|png|gif|mov)$/i.test(item.file.fsName)) {
	...
	function addLocationTextToComp(comp) {
		...
		var xmpFile = new XMPFile(fsName, XMPConst.FILE_UNKNOWN, XMPConst.OPEN_ONLY_XMP);
		var xmp = xmpFile.getXMP();
		var locationProp = xmp.getProperty(XMPConst[selectedXmpNs.selection.text], xmpProperty.text);
	...
	function addBackgroundToComp(comp) {
		...
		var bgLayer;
		if (!isValid(bgSolid)) {
			bgLayer = comp.layers.addSolid([0, 0, 0], bgSolidName, comp.width, comp.height, comp.pixelAspect, comp.duration);
